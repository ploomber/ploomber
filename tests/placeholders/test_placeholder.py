...
def test_error_if_raw_source_cant_be_retrieved():
    ...
    assert "Could not load raw source from jinja2.Template" in str(excinfo.value)
...
def test_error_if_initialized_with_unsupported_type():
    ...
    assert (
        "Placeholder must be initialized with a Template, "
        "Placeholder, pathlib.Path or str, got NoneType instead"
    ) == str(excinfo.value)
...
def test_sql_placeholder_repr_shows_tags_if_unrendered_sql():
    ...
    assert repr(SQLRelationPlaceholder(("{{schema}}", "{{name}}", "table"))) == expected
...
def test_hot_reload_with_template_env(env_init, path_to_test_pkg):
    ...
    query = (
        '{% import "macros.sql" as m %}SELECT * FROM {{m.my_macro()}}' " WHERE x = 10"
    )
...
def test_error_if_missing_upstream():
    ...
    assert 'Cannot obtain upstream dependency "name" for task "task"' in str(
        excinfo.value
    )
...
def test_error_if_no_upstream():
    ...
    msg = (
        'Cannot obtain upstream dependency "name". '
