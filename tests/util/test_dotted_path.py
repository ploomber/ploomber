...
from ploomber.util.dotted_path import create_intermediate_modules, dotted_path_exists
...
def test_call_dotted_path(
    tmp_directory, add_current_to_sys_path, no_sys_modules_cache, kwargs, expected
):
...
"""
    ...
    assert dotted_path.call_dotted_path("my_module.function", kwargs=kwargs) == expected


def test_call_dotted_path_unexpected_kwargs(
...
"""
    ...
    expected = "function() got an unexpected keyword argument 'a' " "(Loaded from:"
...
def test_check_defines_function_with_name(
    tmp_directory, add_current_to_sys_path, no_sys_modules_cache, source, loc_expected
):
...
def test_check_last_definition_is_function(source):
    ...
    with pytest.raises(TypeError) as excinfo:
        dotted_path._check_last_definition_is_function(module, "name", "x.name")

    assert (
        "Failed to load dotted path 'x.name'. "
        "Expected last defined 'name' to be a function. Got:" in str(excinfo.value)
    )
...
def test_check_defines_function_with_name_detects_aliasing(
    ...
    with pytest.raises(NotImplementedError):
        dotted_path._check_defines_function_with_name("function.py", "some_name", None)
...
def test_lazily_locate_dotted_path_error_if_no_package_spec(dotted_path_str):
    ...
    assert (
        f"Error processing dotted path '{dotted_path_str}', no "
        "module named 'a'" in str(excinfo.value)
    )


@pytest.mark.parametrize("dotted_path_str", ["a", "a..b.c"])
def test_lazily_locate_dotted_path_error_if_invalid_dotted_path(dotted_path_str):
    with pytest.raises(ValueError) as excinfo:
        dotted_path.lazily_locate_dotted_path(dotted_path_str)

    expected = (
        f"Invalid dotted path '{dotted_path_str}'. "
        "Value must be a dot "
...
def test_lazily_locate_dotted_path_missing_module(
    tmp_directory, add_current_to_sys_path, no_sys_modules_cache
):
    Path("a").mkdir()
    ...
    assert "No module named 'a.b'. Expected to find one of" in str(excinfo.value)


def test_error_if_doesnt_define_name(
    tmp_directory, add_current_to_sys_path, no_sys_modules_cache
):
    Path("a.py").touch()
    ...
    assert "Failed to locate dotted path 'a.unknown_name'" in str(excinfo.value)
...
"""
    ...
    dp = dotted_path.DottedPath(
        dict(dotted_path="some_module.fn", some_arg=10), lazy_load=False
    )
...
"""
    ...
    expected = (
        "Got duplicated arguments ('some_arg') when calling "
...
def test_load_dotted_path_if_import_fails(path, err_msg, tmp_directory, tmp_imports):
...
def test_load_dotted_path_if_attribute_not_found(
    path, err_msg, root, tmp_directory, tmp_imports
):
    Path("my_module.py").write_text("")
...
def test_create_intermediate_modules_error_if_exists(tmp_directory, tmp_imports):
