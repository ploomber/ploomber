...
def test_entry_point_pkg_location_ignore_egg_info(tmp_directory, method):
    ...
    assert getattr(default, method)() == str(Path("src", "package_b", "pipeline.yaml"))
...
def test_entry_point_pkg_location_multiple_pkgs(tmp_directory, method):
    ...
    assert getattr(default, method)() == str(Path("src", "package_a", "pipeline.yaml"))
...
def test_entry_point_with_name(tmp_directory):
    ...
    assert (
        default.entry_point_with_name(name="pipeline.train.yaml")
        == "pipeline.train.yaml"
    )


def test_entry_point_with_nameuses_pipeline_yaml_if_src_one_is_missing(tmp_directory):
    Path("setup.py").touch()
    Path("pipeline.yaml").touch()
...
def test_path_to_env_loads_file_with_same_name(tmp_directory):
    ...
    assert default.path_to_env_from_spec(Path("dir", "pipeline.train.yaml")) == str(
...
def test_path_to_env_error_if_no_extension():
    ...
    expected = "Expected path to spec to have a file extension " "but got: 'pipeline'"
...
def test_path_to_env_error_if_dir(tmp_directory):
    ...
    expected = (
        "Expected path to spec 'pipeline.yaml' to be a file "
...
def test_find_env_yml_with_name(tmp_directory):
    ...
    assert default.try_to_find_env_yml(Path("dir", "pipeline.train.yaml")) == str(
        Path("env.train.yml").resolve()
    )
...
def test_finds_pipeline_with_name(tmp_directory):
    ...
    assert (
        expected
        == default.find_root_recursively(filename="pipeline.serve.yaml").resolve()
    )
...
def test_pipeline_yaml_with_name_in_root_directory(return_data, monkeypatch):
    ...
    assert (
        "pipeline.serve.yaml cannot be in the filesystem root. "
        "Please add it inside a directory like "
        "project-name/pipeline.serve.yaml. "
    ) in str(excinfo.value)
    assert mock.call_count == len(return_data)
...
def test_error_if_no_pipeline_yaml_and_no_setup_py(tmp_directory):
    ...
    assert (
        "Looked recursively for a setup.py or " "pipeline.yaml in parent folders"
    ) in str(excinfo.value)
...
@pytest.mark.parametrize(
    "root_location, working_dir, to_create, filename",
    [
        ["some/dir/pipeline.yaml", "some/dir", "some/pipeline.yaml", None],
        [
            "some/dir/pipeline.x.yaml",
            "some/dir",
            "some/pipeline.x.yaml",
            "pipeline.x.yaml",
        ],
        [
            "some/dir/another/dir/pipeline.yaml",
...
def test_returns_spec_location_if_setup_py_is_in_a_parent_folder(tmp_directory):
    p = Path("path", "to", "dir", "pipeline.yaml")
    p.parent.mkdir(parents=True)
    p = p.resolve()
    p.touch()
    ...
    assert Path(default.find_root_recursively()).resolve() == p.parent.resolve()
...
"""
    ...
    assert (
        Path(default.entry_point(root_path="dir")).resolve()
        == Path("dir", "pipeline.another.yaml").resolve()
    )
