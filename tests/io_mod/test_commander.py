...
from ploomber.io._commander import (
    Commander,
    CommanderException,
    CommanderStop,
    to_pascal_case,
...
def test_get_template(tmp_directory):
    ...
    with Commander("workspace", templates_path=("test_pkg", "templates")) as cmdr:
...
def test_show_command_on_error():
    ...
    lines = str(excinfo.value).splitlines()
    assert lines[0] == "An error occurred when executing command: pip something"
    assert "returned non-zero exit status 1." in lines[1]
    assert len(lines) == 2
...
def test_commander_custom_environment(tmp_directory):
    ...
    cmdr = Commander(
        ...
        environment_kwargs=dict(variable_start_string="[[", variable_end_string="]]"),
    )

    cmdr.copy_template("square-brackets.sql", placeholder="value")

    assert Path("workspace", "square-brackets.sql").read_text() == "value {{another}}"
