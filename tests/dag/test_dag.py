...
@pytest.mark.parametrize("executor", _executors)
def test_executor_keeps_running_until_no_more_tasks_can_run(executor, tmp_directory):
    dag = DAG(executor=executor)
    t_fail = PythonCallable(failing_root, File("t_fail"), dag, name="t_fail")
    t_fail_downstream = PythonCallable(
        failing, File("t_fail_downstream"), dag, name="t_fail_downstream"
    )
    t_touch_aborted = PythonCallable(
        touch, File("t_touch_aborted"), dag, name="t_touch_aborted"
    )
...
def test_status_on_render_source_fail():
    def make():
        ...
        dag = DAG()
        SQLDump(
            "SELECT * FROM my_table", File("ok.txt"), dag, name="t1", client=mock_client
        )
        t2 = SQLDump(
            "SELECT * FROM {{table}}",
            File("a_file.txt"),
        ...
        SQLDump(
            "SELECT * FROM my_table_2", File("ok_2"), dag, name="t5", client=mock_client
        )
        t2 >> t3 >> t4
        return dag
...
def test_tracebacks_are_shown_for_all_on_build_failing_tasks(executor):
    ...
    assert "PythonCallable: t2 -> File('another_file.txt')" in str(excinfo.getrepr())
...
def test_invalid_start_method_in_parallel_executor(tmp_directory, start_method):
    with pytest.raises(ValueError) as excinfo:
...
def test_switch_start_method_in_parallel_executor(
    tmp_directory, start_method, monkeypatch
):
...
def test_default_start_method_in_parallel_executor(
    tmp_directory, start_method, monkeypatch
):
    dag = DAG(executor=Parallel(processes=2, start_method=start_method))
...
def test_on_render_crashes(tmp_directory):
    ...
    msg = (
        'Exception when running on_render for DAG "No name": crash!'
...
def test_on_finish_crashes(tmp_directory):
    ...
    msg = (
        'Exception when running on_finish for DAG "No name": crash!'
        "\nNeed help? https://ploomber.io/community"
    )
    assert str(excinfo.value) == msg
    assert "crash!" in str(excinfo.getrepr())
...
def test_on_failure_crashes(caplog):
    ...
    msg = (
        'Exception when running on_failure for DAG "dag": crash!'
        "\nNeed help? https://ploomber.io/community"
    )
    assert str(excinfo.value) == msg
    assert "crash!" in str(excinfo.getrepr())
...
def test_build_debug(method, monkeypatch):
    dag = DAG()
    PythonCallable(touch_root, File("file.txt"), dag=dag, name="first", debug_mode=None)
...
def test_outdated_if_different_params(tmp_directory):
    ...
    assert {t.exec_status for t in dag.values()} == {TaskStatus.WaitingExecution}
...
def test_error_if_missing_pypgraphviz(monkeypatch, dag):
    ...
    if sys.version_info < (3, 8):
        assert (
            "'pygraphviz<1.8' is required to use 'plot'. Install "
            "with: pip install 'pygraphviz<1.8'" in str(excinfo.value)
        )
        assert "conda install 'pygraphviz<1.8' -c conda-forge" in str(excinfo.value)
    else:
        assert (
            "'pygraphviz' is required to use 'plot'. "
            "Install with: pip install 'pygraphviz'" in str(excinfo.value)
        )
        assert "conda install pygraphviz -c conda-forge" in str(excinfo.value)
