...
def test_env_repr_and_str(cleanup_env, monkeypatch):
    ...
    d = {"user": "user", "cwd": "cwd", "now": "current-timestamp", "root": "root"}
    assert repr(env) == f"Env({d})"
    assert str(env) == str(d)


def test_env_repr_and_str_when_loaded_from_file(
    tmp_directory, cleanup_env, monkeypatch
):
    mock = Mock()
    mock.datetime.now().isoformat.return_value = "current-timestamp"
    monkeypatch.setattr(expand, "datetime", mock)
...
def test_env_load_env_vars(tmp_directory, cleanup_env, monkeypatch):
    monkeypatch.setenv("TEST_ENV", "test_env_value")

    path_env = Path("env.yaml")

    d = {"test_env": "{{env.TEST_ENV}}"}

    path_env.write_text(yaml.dump(d))
    env = Env()

    assert env["test_env"] == "test_env_value"
...
def test_path_returns_Path_objects(cleanup_env):
    env = Env({"path": {"a": "/tmp/path/file.txt", "b": "/another/path/file.csv"}})
...
def test_init_with_nonexistent_package(cleanup_env):
    ...
    expected = (
        'Could not resolve _module "i_do_not_exist", '
...
def test_init_with_file(tmp_directory, cleanup_env):
    ...
    expected = (
        'Could not resolve _module "not_a_package", '
...
def test_can_instantiate_env_if_located_in_sample_dir(tmp_sample_dir, cleanup_env):
    Env()
...
def test_with_env_initialized_from_path_looks_recursively(cleanup_env, tmp_directory):
    Path("env.yaml").write_text('{"a": 42}')
...
def test_error_if_no_project_root(tmp_directory):
    ...
    assert "An error happened while expanding placeholder {{root}}" in str(
        excinfo.getrepr()
    )

    assert "could not find a setup.py in a parent folder" in str(excinfo.getrepr())
...
def test_replace_nested_flatten_keys_env_dict():
    ...
    assert (
        new_env.a.b == 2 and new_env.a.c == 2 and env is not new_env
    )  # must return a copy
...
def test_expand_raw_dict_nested():
    mapping = EnvDict({"key": "value"})
    d = {"section": {"some_settting": "{{key}}"}, "list": ["{{key}}", "{{key}}"]}
    assert expand_raw_dictionary(d, mapping) == {
        "section": {"some_settting": "value"},
        "list": ["value", "value"],
    }


def test_envdict_git_ignored_if_git_command_fails_and_no_git_placeholder(tmp_directory):
...
def test_iterate_nested_dict(constructor):
    ...
    assert parent is numbers and key == 0 and value == 1, preffix == ["a", "b", "c", 0]

    parent, key, value, preffix = next(g)
    assert parent is numbers and key == 1 and value == 2, preffix == ["a", "b", "c", 1]

    parent, key, value, preffix = next(g)
    assert parent is numbers and key == 2 and value == 3, preffix == ["a", "b", "c", 2]
...
@pytest.mark.parametrize(
    "content, type_", [["a", "str"], ["- a", "list"], ["", "NoneType"]]
)
def test_error_when_loaded_obj_is_not_dict(content, type_, tmp_directory):
    ...
    expected = (
        "Expected object loaded from 'file.yaml' to be "
...
def test_error_message_if_missing_default_placeholder(tmp_directory, value, error, arg):
...
"""
    ...
    env = EnvDict(
        {"meta": {"import_from": "base/base.yaml"}, "here": "{{here}}"},
        path_to_here=os.getcwd(),
    )

    assert Path(env.here).resolve() == Path(tmp_directory).resolve()
    assert Path(env.base_here).resolve() == Path(tmp_directory, "base").resolve()
...
def test_import_from_validates_meta_keys():
    ...
    expected = (
        "Error validating meta, the following keys aren't "
...
def test_import_from_validates_meta_import_from_exists(tmp_directory):
    with pytest.raises(ValidationError) as excinfo:
        EnvDict(
            {"meta": {"import_from": "base.yaml"}, "key": "new_value"},
            path_to_here=os.getcwd(),
        )

    expected = (
        "Expected import_from value 'base.yaml' to be a "
