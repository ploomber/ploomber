...
def dag():
    ...
    t1 = PythonCallable(touch, File("1.txt"), dag=dag, name="without_dependencies")
    t2 = PythonCallable(
        touch_with_upstream,
        File("2.txt"),
        dag=dag,
...
def dag_with_unserializer():
    ...
    t1 = PythonCallable(
        fn_data_frame, File("t1.parquet"), dag, name="t1", serializer=df_serializer
    )
    t2 = PythonCallable(
        fn_adds_one,
        File("t2.parquet"),
...
def test_catches_signature_errors_at_render_time():
    dag = DAG()
    t = PythonCallable(fn, File("file.txt"), dag, "callable", params=dict(non_param=1))
...
def test_load_with_unserializer_function(
    tmp_directory, dag_with_unserializer, monkeypatch
):
    dag_with_unserializer.build()
...
def test_load_from_metaproduct(tmp_directory):
    ...
    task = PythonCallable(touch_meta, {"a": File("a.csv"), "b": File("b")}, dag)
...
def test_unserializes_upstream_metaproduct(tmp_directory):
    ...
    t1 = PythonCallable(
        touch_meta,
        {"one": File("one"), "another": File("another")},
        dag=dag,
        name="first",
    )
...
def test_debug_with_userializer(
    tmp_directory, dag_with_unserializer, monkeypatch, kind, module
):
    mock = Mock()
    monkeypatch.setattr(module, "runcall", mock)
    ...
    assert mock.call_args[0][0] is dag_with_unserializer["t2"].source.primitive
...
def test_validates_debug_mode_in_constructor():
    with pytest.raises(ValueError) as excinfo:
        PythonCallable(touch, File("file.txt"), dag=DAG(), debug_mode="something")
...
def test_debug_later(tmp_directory, monkeypatch, capsys):
    ...
    task = PythonCallable(crash, File("file.txt"), dag=DAG(), debug_mode="later")
