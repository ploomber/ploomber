...
def test_sqldump_does_not_required_product_tag(tmp_directory):
    ...
    # the render step
    SQLDump(
        "SELECT * FROM numbers LIMIT {{limit}}",
        File(out),
        dag,
        name="dump.csv",
        client=client,
...
def test_sql_runtime_params(tmp_directory):
    ...
    # make some data and save it in the db
    df = pd.DataFrame({"number": range(10), "char": list(string.ascii_letters[:10])})
    df.to_sql("data", conn)
...
""",
    ...
    assert list(pd.read_parquet("chars.parquet").char) == ["b", "d", "f", "h", "j"]
...
def test_sql_dump_shows_executed_code_if_fails(tmp_directory):
    ...
    SQLDump("SOME INVALID SQL", File("data.parquet"), dag, name="data", client=client)
...
@pytest.mark.parametrize(
    "product_arg, expected_io_handler",
    [
        ["out.csv", io.CSVIO],
        ["out.ext", io.CSVIO],
...
def test_dump_io_handler(product_arg, expected_io_handler):
    dag = DAG()
    t = SQLDump(
        "SELECT * FROM some_table", File(product_arg), dag, name="dump", client=Mock()
    )
