...
from ploomber.products import File, PostgresRelation, GenericProduct, GenericSQLRelation
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [SQLScript, dict(source="CREATE TABLE {{product}} FROM some_table", kwargs={})],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (SQLScript, GenericSQLRelation(("name", "table")), "CREATE TABLE {{product}}"),
        (SQLDump, GenericProduct("file.txt"), 'SELECT * FROM {{upstream["key"]}}'),
        (
            SQLTransfer,
            GenericSQLRelation(("name", "table")),
...
def test_postgresscript_with_relation(pg_client_and_schema):
    ...
    dag = DAG()
    t = SQLScript(
        "CREATE TABLE {{product}} AS SELECT * FROM {{name}}",
    ...
    assert str(t.source) == "CREATE TABLE user.table AS SELECT * FROM some_table"
...
def test_task_change_in_status(tmp_directory):
    ...
    assert (
        ta.exec_status == TaskStatus.WaitingExecution
...
def test_raises_render_error_if_missing_param_in_code():
    ...
    ta = ShellScript(
        '{{command}} "a" > {{product}}', File("a.txt"), dag, name="my task"
    )
...
def test_raises_render_error_if_missing_param_in_product():
    ...
    ta = ShellScript(
        'echo "a" > {{product}}', File("a_{{name}}.txt"), dag, name="my task"
    )
...
def test_raises_render_error_if_non_existing_dependency_used():
    ...
    tb = ShellScript(
        "cat {{upstream.not_valid}} > {{product}}", File("b.txt"), dag, name="bash2"
    )
    ta >> tb
...
def test_shows_warning_if_unused_dependencies():
    ...
    tb = ShellScript('cat {{upstream["ta"]}} > {{product}}', File("b.txt"), dag, "tb")
    tc = ShellScript('cat {{upstream["tb"]}} > {{product}}', File("c.txt"), dag, "tc")
...
def test_placeholder_is_copied_upon_initialization():
    ...
    t1 = SQLScript(p, PostgresRelation(("schema", "a_table", "table")), dag, name="t1")
    t2 = SQLScript(
        p, PostgresRelation(("schema", "another_table", "table")), dag, name="t2"
    )
...
def test_building_a_single_task_when_has_unrendered_upstream():
    ...
    msg = (
        'Cannot directly build task "2" as it has upstream dependencies'
...
def test_task_build_does_not_upload_if_downloaded(tmp_directory, monkeypatch):
    ...
    monkeypatch.setattr(
        dag["root"].product, "upload", Mock(wraps=dag["root"].product.upload)
    )
...
def test_error_message_when_some_products_are_missing(tmp_directory):
    ...
    dag = DAG(executor=Serial(build_in_subprocess=False))
    t = PythonCallable(
        my_task, {"one": File("one"), "two": File("two"), "three": File("three")}, dag
    )
