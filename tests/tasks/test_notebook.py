...
@pytest.mark.parametrize(
    "name, out_dir, ploomber_eng",
    [
        ["sample.py", ".", False],
        ["sample.R", ".", False],
        ["sample.ipynb", ".", False],
        # check still works even if the folder does not exit yet
        ["sample.ipynb", "missing_folder", False],
        # For Ploomber Engine
        ["sample.py", ".",True],
        pytest.param("sample.R", ".", True, marks=pytest.mark.xfail(reason='Ploomber Engine doesn\'t support R')),
        ["sample.ipynb", ".", True],
        # check still works even if the folder does not exit yet
        ["sample.ipynb", "missing_folder", True],
    ],
)
def test_execute_sample_nb(name, out_dir, ploomber_eng, tmp_sample_tasks):
    dag = DAG()

    NotebookRunner(
        Path(name), product=File(Path(out_dir, name + ".out.ipynb")), dag=dag,
        ploomber_engine = ploomber_eng
    )
    dag.build()


def _dag_simple(ploomber_eng, nb_params=True, params=None, static_analysis="regular"):
    path = Path("sample.py")
...
def _dag_two_tasks(ploomber_eng, nb_params=True, params=None, static_analysis="regular"):
    root = Path("root.py")
    root.write_text(
        """
# + tags=["parameters"]
...
def test_error_on_syntax_error(tmp_directory, error_class, static_analysis, ploomber_eng):
    path = Path("sample.py")
...
"""
    ...
    dag = DAG()
    NotebookRunner(
        path, product=File("out.ipynb"), dag=dag, ploomber_engine=ploomber_eng, static_analysis=static_analysis
    )
...
def test_error_on_undefined_name_error(tmp_directory, error_class, static_analysis, ploomber_eng):
    path = Path("sample.py")
...
"""
    ...
    dag = DAG()
    NotebookRunner(
        path, product=File("out.ipynb"), dag=dag, static_analysis=static_analysis, ploomber_engine=ploomber_eng
    )
...
def test_render_pass_on_missing_product_parameter(tmp_directory, ploomber_eng):
    path = Path("sample.py")
...
"""
    ...
    dag = DAG()
    NotebookRunner(path, product=File("out.ipynb"), dag=dag, ploomber_engine=ploomber_eng)
...
def test_render_error_on_missing_upstream(
    tmp_directory, code, error_class, static_analysis, ploomber_eng
):
    path = Path("sample.py")
...
def test_render_error_on_missing_params(tmp_directory, factory, ploomber_eng):
    dag = factory(ploomber_eng = ploomber_eng, static_analysis="strict")
...
def test_render_error_on_unexpected_params(tmp_directory, factory, ploomber_eng):
    dag = factory(ploomber_eng = ploomber_eng, nb_params=False, params=dict(a=1, b=2, c=3), static_analysis="strict")
...
def test_render_error_on_missing_and_unexpected_params(tmp_directory, factory, ploomber_eng):
    dag = factory(ploomber_eng, nb_params=True, params=dict(d=1, e=2, f=3), static_analysis="strict")
...
def test_ignores_declared_product_and_upstream(tmp_directory, code, ploomber_eng):
    path = Path("sample.py")
...
def test_can_convert_to_html(tmp_sample_tasks, ploomber_eng):
    dag = DAG()

    NotebookRunner(
        Path("sample.ipynb"), product=File(Path("out.html")), dag=dag, name="nb", ploomber_engine=ploomber_eng
    )
    dag.build()
...
def test_can_execute_with_parameters(tmp_directory, ploomber_eng):
    dag = DAG()
...
def test_can_execute_when_product_is_metaproduct(tmp_directory, ploomber_eng):
    dag = DAG()
...
def test_multiple_nb_product_success(product, nb_product_key, nbconvert_exporter_name, ploomber_eng):
    dag = DAG()
...
def test_multiple_nb_product_error(
    product, nb_product_key, nbconvert_exporter_name, expected_error, ploomber_eng
):
    dag = DAG()
...
def test_raises_error_if_key_does_not_exist_in_metaproduct(tmp_directory, ploomber_eng):
    dag = DAG()
...
def test_failing_notebook_saves_partial_result(tmp_directory, ploomber_eng):
    dag = DAG()
...
def test_error_if_wrong_exporter_name(tmp_sample_tasks, ploomber_eng):
    dag = DAG()
...
def test_error_if_cant_find_exporter_name(tmp_sample_tasks, ploomber_eng):
    dag = DAG()
...
def test_skip_kernel_install_check(tmp_directory, ploomber_eng):
    dag = DAG()
...
def test_creates_parents(tmp_directory, ploomber_eng):
    dag = DAG()
...
Path(product['file']).touch()
    ...
    NotebookRunner(code, product=product, dag=dag, ext_in="py", name="nb", ploomber_engine=ploomber_eng)
    dag.build()
...
@pytest.mark.parametrize(
    ...
    [
        [False],
        pytest.param(True, marks=pytest.mark.xfail(reason="Ploomber Engine dont support R")),
    ],
)
def test_debug_error_if_r_notebook(tmp_sample_tasks, ploomber_eng):
    dag = DAG()

    t = NotebookRunner(Path("sample.R"), product=File("out.ipynb"), dag=dag, ploomber_engine=ploomber_eng)
...
def test_hot_reload(tmp_directory, ploomber_eng):
    cfg = DAGConfigurator()
...
1 + 1
    ...
    t = NotebookRunner(
        path, product=File("out.ipynb"), dag=dag, kernelspec_name="python3", ploomber_engine=ploomber_eng
    )
...
def test_warns_if_export_args_but_ipynb_output(tmp_sample_tasks, ploomber_eng):
...
def test_change_static_analysis(tmp_sample_tasks, ploomber_eng):
    dag = DAG(executor=Serial(build_in_subprocess=False))
...
def test_validates_static_analysis_value(tmp_sample_tasks, ploomber_eng):
...
def test_warns_on_unused_parameters(tmp_sample_tasks, ploomber_eng):
    dag = DAG()
    NotebookRunner(Path("sample.ipynb"), File("out.ipynb"), dag=dag, params=dict(a=1), ploomber_engine=ploomber_eng)
...
def test_static_analysis_regular_raises_error_at_runtime_if_errors(tmp_directory, ploomber_eng):
    path = Path("nb.py")
    path.write_text(
        """
# + tags=["parameters"]
...
if
    ...
    dag = DAG()
    NotebookRunner(Path("nb.py"), File("out.ipynb"), dag=dag, static_analysis="regular", ploomber_engine=ploomber_eng)
...
def test_static_analysis_strict_raises_error_at_rendertime_if_errors(tmp_directory, ploomber_eng):
    path = Path("nb.py")
    path.write_text(
        """
# + tags=["parameters"]
...
if
    ...
    dag = DAG()
    NotebookRunner(Path("nb.py"), File("out.ipynb"), dag=dag, static_analysis="strict", ploomber_engine=ploomber_eng)
...
1 + 1
    ...
    NotebookRunner(
        ...
        params=dict(some_param="value"), ploomber_engine=ploomber_eng
    )
...
def test_replaces_existing_product(tmp_directory, ploomber_eng):
    Path("out.html").touch()
...
1 + 1
    ...
    dag = DAG()
    NotebookRunner(Path("nb.py"), File("out.html"), dag=dag, ploomber_engine = ploomber_eng)
...
def test_initialize_with_str_like_path(tmp_directory, ploomber_eng):
    Path("script.py").touch()
    dag = DAG()

    with pytest.raises(ValueError) as excinfo:
        NotebookRunner("script.py", File("out.html"), dag=dag, ploomber_engine=ploomber_eng)
...
def test_uploads_notebook_if_it_fails(tmp_directory, product, expected_remote, ploomber_eng, kwargs):
    path = Path("nb.py")
    path.write_text(
        """
# + tags=["parameters"]
...
raise ValueError("some stuff happened")
    ...
    NotebookRunner(Path("nb.py"), product, dag=dag,ploomber_engine=ploomber_eng, **kwargs)
...
def test_validates_debug_mode_in_constructor(tmp_directory, ploomber_eng):
    path = Path("nb.py")
    path.write_text(
        """
# + tags=["parameters"]
# some code
    ...
    with pytest.raises(ValueError) as excinfo:
        NotebookRunner(
            Path("nb.py"), File("out.html"), dag=DAG(),ploomber_engine=ploomber_eng, debug_mode="something"
        )
...
def test_validates_debug_mode_property(tmp_directory, ploomber_eng):
    path = Path("nb.py")
    path.write_text(
        """
# + tags=["parameters"]
# some code
    ...
    task = NotebookRunner(Path("nb.py"), File("out.html"), dag=DAG(), debug_mode=None, ploomber_engine=ploomber_eng)
