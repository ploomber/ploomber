...
def test_error_if_cannot_infer_from_extension_and_missing_exporter_name():
    ...
    assert "Could not determine format for product 'out.extension'" in str(
        excinfo.value
    )


def test_error_if_cannot_infer_from_extension_suggests_pipeline_yaml(tmp_directory):
...
def test_error_if_missing_exporter_name_and_extension():
    ...
    assert "Could not determine format for product 'file'" in str(excinfo.value)
