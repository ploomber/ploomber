...
def test_from_params_resolves_paths():
    ...
    assert Path(dag["task_group0"].product) == Path("dir", "file-0.txt").resolve()
    assert Path(dag["task_group1"].product) == Path("dir", "file-1.txt").resolve()
...
def test_from_grid_resolve_relative_to():
    ...
    assert str(dag["task_group0"].product) == str(Path("").resolve() / "file-0.txt")
    assert str(dag["task_group1"].product) == str(Path("").resolve() / "file-1.txt")
    assert str(dag["task_group2"].product) == str(Path("").resolve() / "file-2.txt")
    assert str(dag["task_group3"].product) == str(Path("").resolve() / "file-3.txt")
...
def test_metaproduct():
    ...
    group = TaskGroup.from_params(
        PythonCallable,
        File,
        {"one": "one.txt", "another": "another.txt"},
        {"source": touch},
        dag,
        name="task_group",
        params_array=[{"param": 1}, {"param": 2}],
...
def test_metaproduct_with_product_placeholders():
    ...
    group = TaskGroup.from_params(
        PythonCallable,
        File,
        {"one": "param=[[param]].txt", "another": "another.txt"},
        {"source": touch},
        dag,
        name="task_group",
        params_array=[{"param": 1}, {"param": 2}],
...
def test_from_params_resolves_paths_in_metaproduct(tmp_directory):
    ...
    assert (
        Path(dag["task_group0"].product["one"]).resolve() == Path("one-0.txt").resolve()
    )
    assert (
        Path(dag["task_group0"].product["another"]).resolve()
        == Path("another-0.txt").resolve()
    )
    assert (
        Path(dag["task_group1"].product["one"]).resolve() == Path("one-1.txt").resolve()
    )
    assert (
        Path(dag["task_group1"].product["another"]).resolve()
        == Path("another-1.txt").resolve()
    )
...
def test_sql_meta_product():
    ...
    TaskGroup.from_params(
        PythonCallable,
        SQLRelation,
        {"one": ["schema", "one", "table"], "another": ["another", "view"]},
        {"source": touch},
        dag=dag,
        name="task_group",
        params_array=[{"param": 1}, {"param": 2}],
