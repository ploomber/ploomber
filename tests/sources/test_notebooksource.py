...
def new_nb(
    fmt=None, kname=None, klang=None, kdisplay=None, code="default", add_tag=True
):
    if code == "default":
        code = ["1", "2"]
...
def test_read_file(nb_str, ext, expected, tmp_directory):
    ...
    assert source._nb_obj_unrendered.metadata.kernelspec.language == expected_lang
    assert source._nb_obj_unrendered.metadata.kernelspec.name == expected_kernel
...
def test_kernelspec_overrides_nb_kernel_info():
    source = NotebookSource(new_nb(fmt="ipynb"), ext_in="ipynb", kernelspec_name="ir")
...
def test_removes_papermill_metadata():
    source = NotebookSource(
        new_nb(fmt="ipynb"), ext_in="ipynb", kernelspec_name="python3"
    )
...
def test_error_if_kernelspec_name_is_invalid():
    with pytest.raises(NoSuchKernel):
        NotebookSource(
            new_nb(fmt="ipynb"), ext_in="ipynb", kernelspec_name="invalid_kernelspec"
        )
...
def test_nb_str_contains_kernel_info():
    ...
    nb = nbformat.reads(source._nb_str_unrendered, as_version=nbformat.NO_CONVERT)
    assert set(nb.metadata.kernelspec.keys()) == {"display_name", "language", "name"}
...
def test_static_analysis(hot_reload, tmp_directory):
    ...
    params = Params._from_dict({"product": File("output.ipynb"), "a": 1, "b": 2})
...
def test_error_if_strict_and_missing_params():
    source = NotebookSource(
        notebook_ab, ext_in="py", kernelspec_name="python3", static_analysis="strict"
    )
...
def test_error_if_strict_and_passing_undeclared_parameter():
    ...
    params = Params._from_dict(
        {"product": File("output.ipynb"), "a": 1, "b": 2, "c": 3}
    )
...
def test_injects_parameters_on_render(nb_str, ext):
    ...
    params = Params._from_dict({"some_param": 1, "product": File("output.ipynb")})
    s.render(params)
...
@pytest.mark.parametrize(
    "nb, expected",
    [(nb_python_meta(), True), (nb_python_inferred(), True), (nb_R_meta(), False)],
)
...
@pytest.mark.parametrize(
    "code, ext, expected_up, expected_prod",
    [nb_case_1, nb_case_2, nb_case_3, nb_case_4],
)
def test_extract_upstream_from_parameters(code, ext, expected_up, expected_prod):
...
def test_str():
    source = NotebookSource(notebook_ab, ext_in="py")
    source.render(Params._from_dict({"product": File("path/to/file/data.csv")}))
    assert str(source) == ("\na = 1\nb = 2\nproduct = None\na + b")
...
def test_inject_cell(tmp_directory):
    ...
    inject_cell(model, params=Params._from_dict({"product": File("output.ipynb")}))


def test_determine_kernel_name_from_ext(tmp_directory):
    nb = jupytext.reads("", fmt=None)
    assert (
        determine_kernel_name(nb, kernelspec_name=None, ext="py", language=None)
        == "python3"
    )
...
def test_nested_nbs_save_injected_cell_paired_ipynb(tmp_nbs_nested):
    ...
    assert get_injected_cell(jupytext.read(Path("clean/notebooks/clean.ipynb")))
...
def test_nested_nbs_remove_injected_cell_paired_ipynb(tmp_nbs_nested):
    ...
    assert not get_injected_cell(jupytext.read(Path("clean/notebooks/clean.ipynb")))
...
""",
    ...
    expected = f"Cannot use '{method}' if notebook was not " "initialized from a file"
...
def test_add_parameters_cell(
    tmp_directory, code, name, extract_product, extract_upstream, expected, expected_fmt
):
    path = Path(name)
    path.write_text(code)
    ...
    add_parameters_cell(
        name, extract_product=extract_product, extract_upstream=extract_upstream
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["Failed to read notebook from 'nb.ipynb'", dict(path="nb.ipynb")],
    ],
)
