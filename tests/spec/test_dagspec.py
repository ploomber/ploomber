    ...
    def fake_create_engine(*args, **kwargs):
        if "sqlite" in args[0]:
            ...
            return create_engine(
                *args, **kwargs, connect_args=dict(options=f"-c search_path={schema}")
            )
...
@fixture_tmp_dir(_path_to_tests() / "assets" / "pipeline-sql-products-in-source")
...
def test_validation_help_message_dict_nb():
    ...
    assert (
        '"product": {"nb": "products/report.ipynb", ' + '"data": "products/data.csv"}'
    ) in excinfo.value.message
...
def test_python_callables_with_extract_upstream(tmp_directory):
    spec = DAGSpec(
        {
            "tasks": [
                {"source": "test_pkg.callables.root", "product": "root.csv"},
                {"source": "test_pkg.callables.a", "product": "a.csv"},
                {"source": "test_pkg.callables.b", "product": "b.csv"},
            ],
            "meta": {"extract_product": False, "extract_upstream": True},
        }
    )
...
def test_custom_serializer(executor, expected):
    dag = DAGSpec(
        {
            "tasks": [{"source": "test_pkg.callables.root", "product": "root.csv"}],
            "executor": executor,
        }
    ).to_dag()
...
"""
    ...
    spec = DAGSpec({"location": "test_error_if_location_is_not_a_callable.make_dag"})

    with pytest.raises(TypeError) as excinfo:
        spec.to_dag()

    expected = (
        "Error loading dotted path 'test_error_if_"
...
"""
    ...
    expected = (
        "Error calling dotted path 'test_error_if_location_is_"
...
@pytest.mark.parametrize(
    "spec",
    [
        {"tasks": [{"source": "task.py", "product": "out.ipynb"}]},
        {
            "tasks": [
                {
                    "source": "task.py",
        ...
        },
        {
            "tasks": [
                {
                    "source": "task.py",
                    "product": {"nb": "out.ipynb", "data": "data.csv"},
                }
            ]
        },
    ],
...
def test_spec_invalid_glob_pattern(tmp_nbs_no_yaml):
    ...
    assert "Cannot instantiate DAGSpec from files with invalid extensions" in str(
        excinfo.value
    )
...
def _random_date_from(date, max_days, n):
    return [
        date + timedelta(days=int(days)) for days in np.random.randint(0, max_days, n)
    ]


def test_postgres_sql_spec(
    tmp_pipeline_sql, pg_client_and_schema, add_current_to_sys_path, monkeypatch
):
    ...
    # includes that info
    monkeypatch.setattr(
        db.sqlalchemy, "create_engine", create_engine_with_schema(schema)
    )
...
def test_sql_spec_w_products_in_source(
    tmp_pipeline_sql_products_in_source, add_current_to_sys_path
):
    with open("pipeline.yaml") as f:
        dag_spec = yaml.load(f, Loader=yaml.SafeLoader)
...
@pytest.mark.parametrize("spec", ["pipeline-sqlite.yaml", "pipeline-sqlrelation.yaml"])
...
def test_mixed_db_sql_spec(
    tmp_pipeline_sql, add_current_to_sys_path, pg_client_and_schema, monkeypatch
):
    _, schema = pg_client_and_schema
...
def test_initialize_with_lazy_import_with_missing_kernel(tmp_pipeline_r, monkeypatch):
    Path("output").mkdir()
    ...
    monkeypatch.setattr(jupyter_client.kernelspec, "get_kernel_spec", no_kernel)
...
@pytest.mark.parametrize(
    ...
    [
        [
            {"source": "load.py", "product": "data.csv"},
        ],
        {"tasks": [{"source": "load.py", "product": "data.csv"}]},
        {"meta": {}, "tasks": []},
    ],
...
def test_expand_built_in_placeholders(tmp_directory, monkeypatch):
    ...
    assert task["product"]["data"] == str(Path(tmp_directory, "src", "pkg", "data.csv"))


@pytest.mark.parametrize("save_env_yaml", [True, False])
def test_git_placeholder_and_git_not_installed(
    monkeypatch, tmp_directory, save_env_yaml
):
    if save_env_yaml:
        with open("env.yaml", "w") as f:
    ...
    expected = "git is not installed" if save_env_yaml else "Ensure git is installed"
    assert expected in str(excinfo.value)


@pytest.mark.parametrize("save_env_yaml", [True, False])
def test_git_placeholder_and_not_in_git_repository(tmp_directory, save_env_yaml):
    if save_env_yaml:
        with open("env.yaml", "w") as f:
            yaml.dump({"some_tag": "{{git}}"}, f)
    ...
    expected = (
        "could not locate a git repository"
        if save_env_yaml
        else "Ensure git is installed"
    )
    assert expected in str(excinfo.value)
...
@pytest.mark.xfail(
    sys.platform == "win32", reason="Pathlib behaves differently on Windows"
)
def test_extract_variables_from_notebooks(tmp_nbs):
    ...
    products = {name: task.product.to_json_serializable() for name, task in dag.items()}

    expected_dependencies = {"clean": {"load"}, "plot": {"clean"}, "load": set()}
...
def test_source_loader(monkeypatch, tmp_directory, no_sys_modules_cache):
    ...
    spec = DAGSpec(
        {
            "meta": {
                "source_loader": {"path": "templates", "module": "test_pkg"},
                "extract_product": False,
                "extract_upstream": False,
            },
            "tasks": [
...
def test_spec_with_location_error_if_meta(tmp_directory):
    ...
    expected = (
        'Failed to initialize spec. If using the "location" key '
...
def test_import_tasks_from_paths_are_relative_to_the_yaml_spec(tmp_nbs, tmp_path):
...
@pytest.mark.parametrize(
    ...
    [
        [
            "get = 1",
            "Error loading dotted path 'dag_level_client_dotted_path"
    ...
    spec = DAGSpec(
        {
            ...
            "tasks": [
                {"source": "sample.sql", "product": ["name", "table"]},
            ],
            "clients": {"SQLScript": "dag_level_client_dotted_path.get"},
        }
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"dotted_path": "test_sets_clients.get", "a": 1},
    ],
    ids=["str", "dict", "dict-with_kwargs"],
)
def test_sets_clients(
    tmp_sample_tasks, add_current_to_sys_path, no_sys_modules_cache, dotted_path_spec
):
    ...
    spec = DAGSpec(
        {
            ...
            "clients": {"SQLScript": dotted_path_spec},
        }
    )
...
def test_validate_product_default_class_keys():
    ...
    expected = (
        "Error validating product_default_class: "
...
def test_doesnt_warn_if_param_declared_in_env_is_used_in_spec():
    ...
    with pytest.warns(None) as record:
        DAGSpec(
            {
                "tasks": [
                    {"source": "sample.sql", "product": ["{{a}}", "table"]},
                ]
            },
            env=dict(a=1),
        )
...
"""
    ...
    with pytest.warns(None) as record:
        DAGSpec(
            {
                "meta": {"import_tasks_from": "more_tasks.yaml"},
                "tasks": [
                    {"source": "sample.sql", "product": ["name", "table"]},
                ],
            },
...
_spec_upstream_extract = {
    "tasks": [
        ...
        {"source": "downstream.py", "product": "downstream.ipynb"},
    ]
}

_spec_upstream_manual = {
    "meta": {"extract_upstream": False},
    "tasks": [
        {
            "source": "upstream.py",
            "name": "upstream-",
...
@pytest.mark.parametrize("spec", [_spec_upstream_extract, _spec_upstream_manual])
...
_spec_callables = {
    "tasks": [
        ...
        {"source": "sample_source_callables.downstream", "product": "downstream.txt"},
    ]
}

_spec_callables_unserializer = {
    "unserializer": "sample_source_callables.unserializer",
    "tasks": [
        {
            "source": "sample_source_callables.upstream",
            "name": "upstream-",
...
def test_grid_and_upstream_wildcard_callables(
    spec_raw, tmp_directory, add_current_to_sys_path, no_sys_modules_cache
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"dotted_path": "my_testing_module.get_client", "param": 10},
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"dotted_path": "my_testing_module.get_db_client", "param": 10},
    ],
)
def test_lazy_load_product_level_client(
    tmp_directory, tmp_imports, my_testing_module, client_spec
):
...
@pytest.mark.xfail(sys.platform == "win32", reason="This doesnt work on windows")
...
def test_pipeline_yaml_parses_env_yaml_literals(tmp_directory, tmp_imports, literal):
...
def test_dagspec_from_dir_doesnt_assign_name(tmp_directory):
    ...
    dag = DAGSpec(
        {"tasks": [{"source": "dir/script.py", "product": "output.ipynb"}]}
    ).to_dag()
