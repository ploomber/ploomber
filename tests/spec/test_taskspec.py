...
from ploomber.spec.taskspec import (
    TaskSpec,
...
def test_task_class_from_script(tmp_directory, source_str, expected):
    assert (
        task_class_from_source_str(
            source_str, lazy_import=False, reload=False, product=None
        )
        is expected
    )
...
@pytest.mark.parametrize(
    "source_str", [str(Path("file.html")), str(Path("file.json")), str(Path("file.a"))]
)
def test_task_class_from_valid_file_without_path_unsupported_extension(
    ...
    assert "If you meant to import a function, please rename it." in str(excinfo.value)
...
"""
    ...
    assert (
        task_class_from_source_str(
            dotted_path, lazy_import=False, reload=False, product=None
        )
        is PythonCallable
    )
...
def test_task_class_from_source_str_error():
    with pytest.raises(DAGSpecInitializationError):
        task_class_from_source_str(
            "not_a_module.not_a_function", lazy_import=False, reload=False, product=None
        )


def test_task_class_from_source_str_invalid_path():
    with pytest.raises(DAGSpecInitializationError) as excinfo:
        task_class_from_source_str([], lazy_import=None, reload=None, product=None)
...
@pytest.mark.parametrize(
    ...
    [
        [
            {"source": "sample.py", "product": "out.ipynb"},
            NotebookRunner,
        ],
        ...
        [
            {
                "source": "sample-select.sql",
                "product": "file.parquet",
                "client": {"dotted_path": "db.get_client"},
            },
            SQLDump,
        ],
        ...
        [
            {
                "source": "sample-select.sql",
                "product": "file.parquet",
                "client": {"dotted_path": "db.get_client", "a": 1},
            },
            SQLDump,
        ],
        # product_client str
        ...
        [
            {
                ...
                "product": "file.parquet",
                "client": "db.get_client",
                "product_client": {"dotted_path": "db.get_client", "a": 1},
            },
            SQLDump,
        ],
    ],
...
def test_initialization(spec, expected, tmp_sample_tasks, tmp_imports):
    meta = Meta.default_meta({"extract_product": False, "extract_upstream": True})
...
def test_error_on_invalid_value_for_file_product(backup_online, tmp_imports):
    ...
    expected = (
        "Error initializing File with argument 1 "
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "source": "sample.sql",
            "product": {"one": ["name", "table"], "another": "some_invalid_product"},
            "client": "db.get_client",
        },
    ],
)
...
def test_error_if_client_dotted_path_returns_none(tmp_sample_tasks, tmp_imports, key):
...
"""
    ...
    spec[key] = (
        "test_error_if_dotted_path_does_not_return_a_callable" ".some_non_function"
    )
    ...
    expected = (
        "Error loading dotted path 'test_error_if_dotted_path"
...
def test_error_on_invalid_class(backup_spec_with_functions_flat, tmp_imports):
    ...
    expected = (
        "Error validating Task spec (class field): "
        "'unknown_class' is not a valid Task class name"
    )
    assert str(excinfo.value) == expected


def test_error_on_invalid_product_class(backup_spec_with_functions_flat, tmp_imports):
    meta = Meta.default_meta({"extract_product": False})
    ...
    expected = (
        "Error validating Task spec (product_class field): "
...
def test_error_on_invalid_source(backup_spec_with_functions_flat, tmp_imports):
    ...
    expected = (
        "Failed to determine task source 'someinvalidstring'\n"
...
def test_grid(backup_spec_with_functions_flat, tmp_imports, grid_spec):
    ...
    task_group, _ = TaskSpec(grid_spec, meta, project_root=".").to_task(dag=dag)

    assert len(task_group) == 4
    assert str(dag["function-0"].product) == str(Path("some_file-0.txt").resolve())
...
def test_grid_with_hook_lazy_import(backup_spec_with_functions_flat, tmp_imports):
    grid_spec = {
        "source": "my_tasks_flat.raw.function",
        "name": "function-",
        "product": "some_file.txt",
    ...
    TaskSpec(grid_spec, meta, project_root=".", lazy_import=True).to_task(dag=dag)
    ...
    assert all(t.on_render._spec.dotted_path == "hooks.on_render" for t in dag.values())
    assert all(t.on_finish._spec.dotted_path == "hooks.on_finish" for t in dag.values())
    assert all(
        t.on_failure._spec.dotted_path == "hooks.on_failure" for t in dag.values()
    )


def test_grid_with_missing_name(
    backup_spec_with_functions_flat, tmp_imports, grid_spec
):
    del grid_spec["name"]

    with pytest.raises(DAGSpecInitializationError) as excinfo:
        TaskSpec(grid_spec, Meta.default_meta(), project_root=".").to_task(dag=DAG())
...
def test_constructor_deep_copies_spec_and_meta(tmp_directory, tmp_imports):
    ...
    meta = Meta.default_meta(
        {"extract_product": False, "product_default_class": prod_default_class}
    )
    params = {"params": {"a": 1}}
    spec = {"source": "sample.sql", "product": "some_file.txt", "params": params}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            {
                "a": {"dotted_path": "my_module::range_", "n": 5},
            },
            {"a": [0, 1, 2, 3, 4]},
        ],
        [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            {
                "a": {"dotted_path": "my_module::range_", "n": 4},
                "b": [4, 5, 6],
            },
            12,
        ],
        [
            [
                {
                    "a": {"dotted_path": "my_module::range_", "n": 3},
                    "b": [4, 5],
                },
                {
                    "c": {"dotted_path": "my_module::range_", "n": 2},
                    "d": {"dotted_path": "my_module::range_", "n": 3},
                },
            ],
            12,
        ],
...
def test_errors_in_grid_with_dotted_paths(
    tmp_directory, tmp_imports, grid, error_expected
):
    Path("my_module.py").write_text(
        """
def return_none():
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            {"a": {"dotted_path": "some_functions::numbers"}},
            {"a": [1, 2, 3]},
        ],
        [
...
def test_process_dotted_paths_errors(tmp_directory, spec, tmp_imports, error_class):
    Path("some_functions.py").write_text(
        """
def numbers():
    return [1, 2, 3]
