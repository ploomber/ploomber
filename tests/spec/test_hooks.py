...
def test_dag_on_finish_with_params(tmp_directory, tmp_imports,
...
def test_task_on_finish_with_params(tmp_directory, tmp_imports,
                                    write_task_hooks_spec):
...
def test_task_on_render_with_params(tmp_directory, tmp_imports,
                                    write_task_hooks_spec):
    dag = DAGSpec('pipeline.yaml').to_dag()
    dag.executor = Serial(build_in_subprocess=False)
...
def test_task_on_failure_with_params(tmp_directory, tmp_imports,
                                     write_task_hooks_spec):
    Path('my_module.py').write_text("""
def touch(product):
    raise Exception
