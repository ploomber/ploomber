        ...
        "tasks": [
            {
                ...
                "product": "output/param=[[param]].ipynb",
                "grid": {"param": [1, 2]},
                "params": {"another": 100, "one-more": 200},
            },
            {"source": "downstream.py", "product": "downstream.ipynb"},
        ]
    }
...
def test_grid_with_params(sample_dagspec_with_params):
    ...
    assert dag["upstream-0"].params == {"param": 1, "another": 100, "one-more": 200}
    assert dag["upstream-1"].params == {"param": 2, "another": 100, "one-more": 200}


def test_with_with_placeholder_task_names(sample_dagspec_with_placeholder_task_names):
    dag = DAGSpec("pipeline.yaml").to_dag()
