...
def _mock_metadata(**kwargs):
    default = dict(
        timestamp=datetime.now().timestamp(), branch=examples._DEFAULT_BRANCH
    )
...
def test_error_if_exception_during_execution(monkeypatch):
    ...
    monkeypatch.setattr(examples, "main", Mock(side_effect=ValueError("some error")))
    ...
    assert "An error happened when executing the examples" in str(excinfo.value)
...
def test_click_exception_isnt_shadowed_by_runtime_error(monkeypatch):
    ...
    # the cli module inside the cli function so we can no longer patch it
    monkeypatch.setattr(
        examples, "main", Mock(side_effect=ClickException("some click exception"))
    )
...
def test_clones_if_corrupted_metadata(clone_examples, tmp_directory, monkeypatch):
    # corrupt metadata
    not_json = Path(tmp_directory, "not.json")
    not_json.write_text("hello")
    monkeypatch.setattr(examples._ExamplesManager, "path_to_metadata", not_json)
...
def test_copy_example(clone_examples, tmp_directory):
    ...
    assert Path(tmp_directory, "templates/ml-online", "src", "ml_online").is_dir()
...
def test_error_unknown_example(tmp_directory, clone_examples):
    ...
    result = runner.invoke(
        cli.cli, ["examples", "--name", "not-an-example", "--output", "some-directory"]
    )
...
def test_error_if_already_exists(clone_examples, tmp_directory):
    ...
    runner = CliRunner()
    result = runner.invoke(cli.cli, ["examples", "--name", "templates/ml-online"])

    expected = (
        "'templates/ml-online' already exists in the current working "
...
def test_did_you_mean_feature(input_name, expected_name):
    runner = CliRunner()
    result = runner.invoke(cli.cli, ["examples", "--name", input_name])
    # assert result.exit_code == 1
