...
def test_entry_point_from_factory_in_environment_variable(backup_test_pkg, monkeypatch):
    monkeypatch.setattr(sys, "argv", ["python"])
...
def test_shows_default_value_from_env_var(tmp_directory, monkeypatch, capsys):
    ...
    captured = capsys.readouterr()
    assert re.search(
        r"defaults\s+to\s+dag.yaml\s+\(ENTRY_POINT\s+env\s+var\)", captured.out
    )
...
def test_log(tmp_nbs, monkeypatch):
    mock = Mock()
    monkeypatch.setattr(
        sys, "argv", ["python", "--log", "info", "--entry-point", "pipeline.yaml"]
    )
...
def test_log_file(tmp_nbs, monkeypatch, opt):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["python", "--log", "info", opt, "my.log", "--entry-point", "pipeline.yaml"],
    )
    monkeypatch.setattr(parsers, "logging", mock)
