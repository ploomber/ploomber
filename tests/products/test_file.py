...
def _make_dag(with_client=True):
    ...
    task = PythonCallable(_touch_upstream, File("file.txt"), dag=dag, name="task")
    root >> task
    return dag
...
def _make_dag_with_metaproduct(with_client=True):
    ...
    task = PythonCallable(
        _touch_upstream,
        {"one": File("file.txt"), "another": File("another.txt")},
        dag=dag,
        name="task",
    )
    root >> task
...
def test_error_on_corrupted_metadata(tmp_directory):
    ...
    msg = (
        "Error loading JSON metadata for File('corrupted') "
...
@pytest.mark.skipif(
    sys.platform == "win32", reason="Windows has a different path representation"
)
...
def test_build_triggers_metadata_download(tmp_directory):
    ...
    client.download.assert_called_with(
        Path(".file.txt.metadata"), destination=Path(".file.txt.metadata.remote")
    )
...
def test_downloads_if_missing_some_products_in_metaproduct(tmp_directory, tmp_path):
    dag = _make_dag_with_metaproduct(with_client=True)
    dag.build()
...
def test_task_with_skipped_and_waiting_to_download_upstream_downloads(tmp_directory):
...
def test_upload_error_if_metadata_or_product_do_not_exist(to_touch, tmp_directory):
    for f in to_touch:
        Path(f).touch()
...
def test_do_not_download_if_metadata_does_not_exist_in_remote(to_touch, tmp_directory):
...
class FileWithUploadCounter(File):
    def upload(self):
        if not Path(".file.txt.metadata").exists():
            ...
            raise ValueError("By the time File.upload is called, metadata must exist")
...
def test_task_build_calls_download_if_remote_up_to_date_products(
    ...
    monkeypatch.setattr(File, "download", Mock(wraps=dag["root"].product.download))
    dag["root"].build()
...
def test_task_build_raises_error_if_upstream_do_not_exist_in_remote(
    ...
    expected = (
        "Cannot build task 3 because the following upstream "
...
def test_task_build_downloads_upstream_if_up_to_date_then_executes(
    ...
    dag = _make_dag_with_upstream().render()
    monkeypatch.setattr(
        dag["root"].product, "download", Mock(wraps=dag["root"].product.download)
    )
    monkeypatch.setattr(dag[3].product, "download", Mock(wraps=dag[3].product.download))
...
def test_dag_build_calls_download_if_remote_up_to_date_products(
    ...
    monkeypatch.setattr(
        dag["task"].product, "download", Mock(wraps=dag["task"].product.download)
    )
