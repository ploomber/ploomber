...
def test_unquoted_relation_to_str():
    assert str(static_analysis.sql.ParsedSQLRelation(None, 'data',
                                                     'table')) == 'data'


def test_unquoted_relation_w_schema_to_str():
    assert str(static_analysis.sql.ParsedSQLRelation('schema', 'data',
                                                     'table')) == 'schema.data'
...
def test_quoted_relation_w_schema_to_str():
    assert (str(
        static_analysis.sql.ParsedSQLRelation('"schema"', '"data"',
                                              'table')) == '"schema"."data"')


def test_detects_create_table_w_schema():
    ct = static_analysis.sql.created_relations(
...
def test_parsing_create_statement(sql, schema, name, kind):
    ...
    assert relations[0] == static_analysis.sql.ParsedSQLRelation(schema=schema,
                                                                 name=name,
                                                                 kind=kind)
...
def test_parses_create_table():
    rels = static_analysis.sql.created_relations(
        'CREATE TABLE x; SELECT * FROM y')
    assert rels[0] == static_analysis.sql.ParsedSQLRelation(schema=None,
                                                            name='x',
                                                            kind='table')
...
def test_repr():
    rel = static_analysis.sql.ParsedSQLRelation(schema=None,
                                                name='name',
                                                kind='view')
...
def test_equality_sql_products(class_):
    ...
    r = static_analysis.sql.ParsedSQLRelation(schema='schema',
                                              name='name',
                                              kind='table')
