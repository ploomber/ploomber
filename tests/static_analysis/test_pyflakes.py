...
def test_check_source_warns_on_unexpected_error(monkeypatch):
    with monkeypatch.context() as m:
        m.setattr(pyflakes.pyflakes_api.ast, "parse", Mock(side_effect=ValueError))
    ...
    assert len(record) == 1
    expected = (
        "An unexpected error happened when analyzing code: ': "
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            dict(a=1),
            "b = None",
            "Missing params: 'b' (to fix this, pass 'b' in the 'params' " "argument).",
...
"""
    ...
    assert len(record) == 1
    assert (
        "Parameters declared in " "the 'parameters' cell do not match task params"
    ) in record[0].message.args[0]


@pytest.mark.parametrize(
    "passed, params_source",
    [
        [set(), "raise Exception"],
        [
            set(),
...
def test_check_notebook_warns_if_pyflakes_error(code, msg):
    ...
    with pytest.warns(UserWarning) as record:
        pyflakes.check_notebook(nb, {}, "file.py", raise_=False, check_signature=True)
...
def test_check_notebook_warns_with_check_signature_disabled(code, msg):
    ...
    with pytest.warns(UserWarning) as record:
        pyflakes.check_notebook(nb, {}, "file.py", raise_=False, check_signature=False)
