...
def test_init(tmp_directory, mock_client):
    client = GCloudStorageClient(
        "my-bucket-name", parent="folder", arg=1, path_to_project_root="."
    )
...
def test_from_service_account_json(
    tmp_directory, mock_client, relative_to_project_root
):
    client = GCloudStorageClient(
        "my-bucket-name",
        "folder",
    ...
    expected = (
        ...
        else Path(tmp_directory, "my.json").resolve()
    )
...
def test_underscore_upload(tmp_directory, mock_client):
    ...
    client = GCloudStorageClient("my-bucket-name", parent="", path_to_project_root=".")
...
def test_upload_file(monkeypatch, parent, mock_client):
    mock = Mock()
    client = GCloudStorageClient(
        "my-bucket-name", parent=parent, path_to_project_root="."
    )
...
def test_download(monkeypatch, parent, mock_client):
    ...
    mock.assert_called_once_with("file.txt", str(PurePosixPath(parent, "file.txt")))
...
def test_error_when_downloading_non_existing(monkeypatch, mock_client):
    ...
    assert "Could not download 'non-existing-file' using client" in str(excinfo.value)
...
def test_download_folder(tmp_directory, monkeypatch, mock_client):
    ...
    mock = Mock()
    client = GCloudStorageClient(
        "my-bucket-name", parent="backup", path_to_project_root="."
    )
    monkeypatch.setattr(client, "_download", mock)
    ...
    mock_blobs[2].download_to_filename.assert_called_once_with(Path("dir/subdir/c"))
...
def test_download_with_custom_destination(monkeypatch, mock_client):
    ...
    mock.assert_called_once_with(
        "another.txt", str(PurePosixPath("parent", "file.txt"))
    )


def test_pickle(mock_client):
    c = GCloudStorageClient("my-bucket-name", parent="", path_to_project_root=".")
    pickle.loads(pickle.dumps(c))


def test_pickle_from_service_acccount(mock_client):
    c = GCloudStorageClient(
        "my-bucket-name", "folder", "my.json", path_to_project_root="."
    )
    pickle.loads(pickle.dumps(c))


def test_close(mock_client):
    GCloudStorageClient("my-bucket-name", parent="", path_to_project_root=".").close()
...
def test_remote_exists_directory(monkeypatch, mock_client):
    ...
    bucket.client.list_blobs.assert_called_once_with(
        "my-bucket-name", prefix="backup/a"
    )
