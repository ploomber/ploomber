...
def test_upload_folder(s3, tmp_directory):
    ...
    client = S3Client("some-bucket", parent="my-folder", path_to_project_root=".")
...
def test_download(s3, tmp_directory, destination, expected):
    ...
    s3.upload_file("dir/a", "some-bucket", str(PurePosixPath("my-folder", "dir/a")))
    path.unlink()
...
def test_download_folder(s3, tmp_directory):
    ...
    for path in chain(iglob("dir/*"), iglob("dir/*/*"), iglob("dir/*/*/*")):
        ...
        if path.is_file():
            s3.upload_file(
                str(path), "some-bucket", str(PurePosixPath("my-folder", *path.parts))
            )
...
def test_close():
    S3Client("some-bucket", parent="my-folder", path_to_project_root=".").close()
