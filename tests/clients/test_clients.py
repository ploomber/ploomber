...
def test_dbapiclient_split_source(tmp_directory):
    client = DBAPIClient(sqlite3.connect, dict(database="my_db.db"), split_source=";")
    client.execute(
...
def test_custom_create_engine_kwargs(monkeypatch):
    ...
    client = SQLAlchemyClient(
        "sqlite:///my_db.db", create_engine_kwargs=dict(key="value")
    )
...
def test_send_more_than_one_command_in_sqlite(code, split_source, tmp_directory):
...
def test_remote_shell_read_file(monkeypatch, tmp_directory):
    ...
    monkeypatch.setattr(
        shell, "ssh_client_and_policy", lambda: (mock_ssh_client, Mock())
    )
    monkeypatch.setattr(shell.tempfile, "mkstemp", lambda: (None, "my_tmp_file"))
    monkeypatch.setattr(shell.os, "close", lambda _: None)
    mock_ssh_client.open_sftp().get.side_effect = lambda x, y: Path(
        "my_tmp_file"
    ).write_text("some content")
    ...
    assert ftp.get.call_args.assert_called_with("/path/to/remote/file", "my_tmp_file")
...
def test_remote_shell_write_to_file(monkeypatch, tmp_directory):
    ...
    assert ftp.put.call_args.assert_called_with("my_tmp_file", "/path/to/remote/file")
    ftp.close.assert_called_once()
