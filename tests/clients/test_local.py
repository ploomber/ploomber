...
def test_error_when_downloading_non_existing(tmp_directory):
    ...
    expected = (
        "Could not download 'file' using client "
...
def test_creates_directory(tmp_directory):
    LocalStorageClient(str(Path("my", "backup", "dir")), path_to_project_root=".")
...
def test_outdated_task_with_metaproduct_downloads_metadata(tmp_directory):
    def _make():
        ...
        dag.clients[File] = LocalStorageClient("backup", path_to_project_root=".")
        t1 = PythonCallable(
            _touch_many, {"one": File("one"), "two": File("two")}, dag, name="task"
        )
        t2 = PythonCallable(_touch_upstream, File("three"), dag, name="another")
        t1 >> t2
        return dag
