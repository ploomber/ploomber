...
-   Performance improvements, DAG does not fetch metadata when it
    doesn\'t need to
-   Factory functions: Bool parameters with default values are now
    represented as flags when called from the CLI
-   CLI arguments to replace values from [env.yaml]{.title-ref} are now
    built with double hyphens instead of double underscores
-   `NotebookRunner` creates parent folders for output file if they
    don\'t exist
-   Bug fixes
...
-   \"Docstrings\" are parsed from notebooks and displayed in DAG status
    table (\#242)
...
-   Fixes [ploomber task]{.title-ref} forced run
...
-   Adds Jupyter notebook extension to inject parameters when opening a
    task
-   Improved CLI [ploomber new]{.title-ref}, [ploomber add]{.title-ref}
    and [ploomber entry]{.title-ref}
-   Spec API documentation additions
-   Support for [on_finish]{.title-ref}, [on_failure]{.title-ref} and
    [on_render]{.title-ref} hooks in spec API
...
-   General clean ups for old [\_\_str\_\_]{.title-ref} and
    [\_\_repr\_\_]{.title-ref} in several modules
...
-   Task.name is now optional when Task is initialized with a source
    that has \_\_name\_\_ attribute (Python functions) or a name
