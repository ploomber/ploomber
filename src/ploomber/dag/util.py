...
def _generate_error_message(duplicated):
    return "\n".join(
        _generate_error_message_pair(key, value) for key, value in duplicated.items()
    )
...
def flatten_products(elements, require_file_client=True):
    ...
    for prod in elements:
        ...
        elif (isinstance(prod, File) and prod.client and require_file_client) or (
            isinstance(prod, File) and not require_file_client
        ):
            flat.append(prod)
...
def _get_parent_from_product(product, base_dir):
    ...
    if path.is_absolute():
        try:
            ...
            raise ValueError(
                "Absolute product paths " f"are not supported: {str(path)!r}"
            ) from e
...
def extract_product_prefixes(dag, base_dir=None):
    files = set(_get_parent_from_product(p, base_dir) for p in iter_file_products(dag))
