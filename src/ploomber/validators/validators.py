...
def validator(fn):
    ...
    def wrapped(**kwargs):
        if "assert_" in kwargs:
            raise TypeError(
                "Do not include the assert_ parameter in " "validator functions"
            )

        if "data" in kwargs:
            raise TypeError(
                "Do not include the data parameter in " "validator functions"
            )
...
def validate_values(assert_, data, values):
    ...
    for column, (kind, params) in values.items():
        if column not in data_cols:
            ...
            msg = (
                '(validate_values) Expected unique values of  "{}" to be a'
            ...
            if len(params) != 2:
                raise ValueError(
                    "If kind is range, you must provide two "
            ...
            msg = (
                '(validate_values) Expected range of "{}" to be [{}, {}], '
                "got [{}, {}]".format(column, min_expected, max_expected, min_, max_)
            )
